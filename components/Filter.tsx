  ...
  const handleColor = (color: string) => {
    ...
    if (filterValue.textValue !== "") {
      ...
      filterColor = props.creatives.filter((creative: any) => {
        ...
        const matchesText =
          creative.title.includes(filterValue.textValue) ||
          creative.subtitle.includes(filterValue.textValue);
  ...
  const handleSearch = (text: string) => {
    ...
    if (filterValue.backgroundColorValue !== "") {
      ...
      filterTextResult = props.creatives.filter((creative: any) => {
        const matchesColor =
          creative.backgroundColor === filterValue.backgroundColorValue;
        // Check if the creative's title or subtitle includes the search text
        const matchesText =
          creative.title.includes(text) || creative.subtitle.includes(text);
        // Return true if both conditions are met
        if (matchesColor && matchesText) return creative;
      });
    } else {
            ...
            {props.colors !== undefined &&
              props.colors.map((color: string) => (
                <div
                  ...
                  style={{
                    backgroundColor:
                      filterValue.backgroundColorValue == color
                        ? "black"
                        : "white",
                  }}
                >
